...
- **`process.nextTick()`**: Fires immediately after the current operation completes, BEFORE the event loop continues. Higher priority than microtasks.
- **`setImmediate()`**: Fires on the next iteration of the event loop (in the "check" phase).
- **Rule of thumb**: `process.nextTick` fires faster, but overusing it can starve I/O.
...
| Tree Shaking | No | Yes (unused exports removed by bundlers) |
| Top-level await | No | Yes |
| Node.js support | Default | Requires `"type": "module"` in `package.json` or `.mjs` extension |

### Buffer
- Buffers are used to work with binary data directly (file I/O, network protocols, crypto).
- Allocated outside V8's heap, managed by Node's C++ layer.
...
- **Use Case**: Chat applications, live dashboards, real-time collaboration.
...
| Caching | Easy (HTTP caching) | Complex (needs client-side cache like Apollo) |
| Learning curve | Low | Medium |

## Additional Interview Questions

### Q: What is the N+1 query problem?
A: When fetching a list of N items, then making 1 additional database query for each item to get related data (e.g., fetching 100 orders, then 100 separate queries for each order's user). Solved by using JOINs, eager loading, or DataLoader (batching + caching).

### Q: How is Node.js different from traditional multi-threaded servers?
A: Traditional servers (Apache) create a new thread per request. Node.js uses a single thread with an event loop for I/O operations, making it highly efficient for I/O-bound tasks. For CPU-bound tasks, Node.js provides Worker Threads.
```
