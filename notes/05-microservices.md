...
- Separates **read** operations (Queries) from **write** operations (Commands) into different models, often backed by different databases.
- **Write side**: Optimized for data integrity (normalized SQL database).
- **Read side**: Optimized for fast queries (denormalized views, Redis, Elasticsearch).
- Events sync the write model to the read model asynchronously.
- **Use Case**: Systems with very different read/write patterns (e.g., an e-commerce product catalog read heavily but written rarely).

```
[Client] --Command--> [Write Model (MySQL)] --Event--> [Event Bus]
                                                          |
[Client] <--Query--- [Read Model (Elasticsearch)] <-------'
```

### Strangler Fig Pattern
- A migration strategy to incrementally replace a monolithic system with microservices.
- Named after the strangler fig tree that grows around an existing tree and eventually replaces it.
- **Steps**:
  1. Put a facade (API Gateway) in front of the monolith.
  2. Build new features as microservices behind the same gateway.
  3. Gradually route traffic from monolith endpoints to microservice endpoints.
  4. Eventually decommission the monolith.
- **Key Benefit**: Zero big-bang risk. Both old and new systems run simultaneously.

### Service Mesh (Istio / Linkerd)
- An infrastructure layer that handles service-to-service communication.
- Deployed as sidecar proxies alongside every service instance.
- **Features**:
  - **mTLS**: Automatic mutual TLS for all inter-service traffic.
  - **Traffic Management**: Canary deployments, A/B testing, fault injection.
  - **Observability**: Automatic distributed tracing, metrics, and access logging.
  - **Retry/Timeout Policies**: Configured at the mesh level, not in application code.
- **When to use**: When you have many microservices (50+) and need consistent cross-cutting concerns without modifying each service's code.

### Distributed Tracing (OpenTelemetry / Jaeger)
- Tracks a single request as it flows across multiple microservices.
- Each service adds a **span** (a unit of work with timing data) to a **trace** (the full request path).
- A unique **trace ID** is propagated via HTTP headers across all services.

```javascript
// OpenTelemetry basic setup (Node.js)
...
async function createOrder(orderData) {
  ...
  try {
    ...
    const result = await db.insert(orderData);
...
- A single entry point for all client requests. Routes, authenticates, and rate-limits.
- **Backend for Frontend (BFF)**: Separate API gateways for different clients (web, mobile, IoT), each tailored to that client's needs.
- **Aggregation**: Gateway combines responses from multiple backend services into a single response for the client.
...
A:
- **Start with a Monolith** when: team is small, domain is unclear, speed of initial development matters.
- **Move to Microservices** when: teams need to deploy independently, different parts need independent scaling, the monolith becomes too large to manage.
- "Don't start with microservices unless you have a very good reason." — Martin Fowler.

### Q: How do you handle distributed logging?
A: Use centralized logging (ELK Stack: Elasticsearch + Logstash + Kibana, or Loki + Grafana). Each service logs in structured JSON format with a **correlation ID** (same as the trace ID) so you can search all log entries for a single request across all services.

### Q: What is the Sidecar pattern?
A: A helper container that runs alongside the main application container in the same pod (in Kubernetes). It handles orthogonal concerns like logging, monitoring, proxy, or configuration sync without modifying the main application. Service meshes use this pattern extensively.
